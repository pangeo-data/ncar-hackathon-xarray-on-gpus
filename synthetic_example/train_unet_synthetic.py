...
from SyntheticDataset import ERA5Dataset, PyTorchERA5Dataset
...
def main():
    ...
    if distributed:
        ...
        train_loader = DataLoader(
            train_pytorch, batch_size=batch_size, pin_memory=True, sampler=train_sampler
        )
    else:
        train_loader = DataLoader(
    ...
    for epoch in range(num_epochs):
        ...
        if distributed:
            total_samples = (
                len(train_loader.dataset) * batch_size * WORLD_SIZE * num_epochs
            )
        else:
